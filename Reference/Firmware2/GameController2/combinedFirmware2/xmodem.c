...
long xmodemReceive()// int (*write)(unsigned char* buffer, int size) )
{
	...
	unsigned char pktsize=128;	// default packet size is 128 bytes
	...
	while(retry > 0)
	{
			...
			xmodemCrcCheck(crcflag, &xmbuf[3], pktsize) )	// packet is not corrupt
		{
			...
			if(xmbuf[1] == seqnum)
			{
				...
				EEPROMWritePage(totalbytes, &xmbuf[3], pktsize); //totalbytes will effectively work as an address into EEPROM
				totalbytes += pktsize;
