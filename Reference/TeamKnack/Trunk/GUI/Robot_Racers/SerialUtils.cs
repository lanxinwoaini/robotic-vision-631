...
namespace Robot_Racers
{
    ...
    public class SerialUtils
    {
        ...
        private void ProcessState(SerialHeader header, byte[] data)
        {
            ...
            if (TheKnack.racer != null && TheKnack.racer.xnaRoboRacerDisplay1 != null && TheKnack.racer.xnaRoboRacerDisplay1.KnackTruck() != null)
                TheKnack.racer.xnaRoboRacerDisplay1.KnackTruck().updateVehicleStateVariables(RobotRacer.stateVariables);
        }
        ...
        private void ProcessRegister(SerialHeader header, byte[] data)
        {
            ...
            try
            {
                ...
                {
                    if (regId >= (ushort)Register.RegisterIds.H_HIGH_ORANGE && regId <= (ushort)Register.RegisterIds.CONV_THRESHOLD_ORANGE)
                    {
                        int regValueInt = BitConverter.ToInt32(reverseBytes(data, 2, 5), 0);
                        TheKnack.racer.updateHSVSlider(regId, regValueInt);
                    }
                    else
                    {
                        int regValueInt = BitConverter.ToInt32(reverseBytes(data, 2, 5), 0);
