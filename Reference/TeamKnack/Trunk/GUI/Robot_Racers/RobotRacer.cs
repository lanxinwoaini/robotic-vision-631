    ...
    public partial class RobotRacer : Form
    {
        ...
        private void courseTabControl_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (courseTabControl.SelectedTab.Name.Equals("courseGraphicTab"))
            {
                ...
                courseGraphicalPanel.Invalidate();
            }
        }
        ...
        private void resetGUICallback()
        {
            ...
            speedSlider.Value = 5;
            setHSVSliders();
            if (TheKnack.racer.xnaRoboRacerDisplay1 != null) TheKnack.racer.xnaRoboRacerDisplay1.resetTruckLocation();
        }
        ...
        {
            currentCourse = new Course(courseTextBox.Text);
            courseGraphicalPanel.repaintGraphics();
        }
        ...
        public void updateProcessedImage(int numPylons, byte[] data)
        {
            ...
            else if (xnaRoboRacerDisplay1 != null && xnaRoboRacerDisplay1.PylonGraphics() != null)
            {
                xnaRoboRacerDisplay1.PylonGraphics().setVisiblePylonData(
                    pylons,
                    xnaRoboRacerDisplay1.KnackTruck().ModelPosition(),
                    xnaRoboRacerDisplay1.KnackTruck().ModelRotation());
            }
        ...
        private void hHighSlider_Scroll(object sender, EventArgs e)
        {
            hRangeLabel.Text = hLowSlider.Value + " - " + hHighSlider.Value;

            SerialHeader header = new SerialHeader();
            header.Type = (byte)SerialUtils.TransmissionType.REGISTER;
            header.Subtype = (byte)SerialUtils.RegisterSubtype.SET_INT;
            ushort regId = (ushort)Register.RegisterIds.H_HIGH_ORANGE;
            if (selectedColorHSV == PylonColor.Green) regId = (ushort)Register.RegisterIds.H_HIGH_GREEN;
            RobotRacer.serialPort.transmitRegData(header, regId, hHighSlider.Value);
        }

        private void hLowSlider_Scroll(object sender, EventArgs e)
        {
            hRangeLabel.Text = hLowSlider.Value + " - " + hHighSlider.Value;
            ...
            header.Type = (byte)SerialUtils.TransmissionType.REGISTER;
            header.Subtype = (byte)SerialUtils.RegisterSubtype.SET_INT;
            ushort regId = (ushort)Register.RegisterIds.H_LOW_ORANGE;
            if (selectedColorHSV == PylonColor.Green) regId = (ushort)Register.RegisterIds.H_LOW_GREEN;
            RobotRacer.serialPort.transmitRegData(header, regId, hLowSlider.Value);
        }

        private void sHighSlider_Scroll(object sender, EventArgs e)
        {
            sRangeLabel.Text = sLowSlider.Value + " - " + sHighSlider.Value;

            SerialHeader header = new SerialHeader();
            header.Type = (byte)SerialUtils.TransmissionType.REGISTER;
            header.Subtype = (byte)SerialUtils.RegisterSubtype.SET_INT;
            ushort regId = (ushort)Register.RegisterIds.S_HIGH_ORANGE;
            if (selectedColorHSV == PylonColor.Green) regId = (ushort)Register.RegisterIds.S_HIGH_GREEN;
            RobotRacer.serialPort.transmitRegData(header, regId, sHighSlider.Value);
        }

        private void sLowSlider_Scroll(object sender, EventArgs e)
        {
            sRangeLabel.Text = sLowSlider.Value + " - " + sHighSlider.Value;
            ...
            header.Type = (byte)SerialUtils.TransmissionType.REGISTER;
            header.Subtype = (byte)SerialUtils.RegisterSubtype.SET_INT;
            ushort regId = (ushort)Register.RegisterIds.S_LOW_ORANGE;
            if (selectedColorHSV == PylonColor.Green) regId = (ushort)Register.RegisterIds.S_LOW_GREEN;
            RobotRacer.serialPort.transmitRegData(header, regId, sLowSlider.Value);
        }

        private void vHighSlider_Scroll(object sender, EventArgs e)
        {
            vRangeLabel.Text = vLowSlider.Value + " - " + vHighSlider.Value;

            SerialHeader header = new SerialHeader();
            header.Type = (byte)SerialUtils.TransmissionType.REGISTER;
            header.Subtype = (byte)SerialUtils.RegisterSubtype.SET_INT;
            ushort regId = (ushort)Register.RegisterIds.V_HIGH_ORANGE;
            if (selectedColorHSV == PylonColor.Green) regId = (ushort)Register.RegisterIds.V_HIGH_GREEN;
            RobotRacer.serialPort.transmitRegData(header, regId, vHighSlider.Value);
        }

        private void vLowSlider_Scroll(object sender, EventArgs e)
        {
            vRangeLabel.Text = vLowSlider.Value + " - " + vHighSlider.Value;
            ...
            header.Type = (byte)SerialUtils.TransmissionType.REGISTER;
            header.Subtype = (byte)SerialUtils.RegisterSubtype.SET_INT;
            ushort regId = (ushort)Register.RegisterIds.V_LOW_ORANGE;
            if (selectedColorHSV == PylonColor.Green) regId = (ushort)Register.RegisterIds.V_LOW_GREEN;
            RobotRacer.serialPort.transmitRegData(header, regId, vLowSlider.Value);
        }
        ...
        private void orangeRadio_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                selectedColorHSV = PylonColor.Orange;
                setHSVSliders();
            }
        }
        ...
        private void greenRadio_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                selectedColorHSV = PylonColor.Green;
                setHSVSliders();
            }

        }

        private void setHSVSliders()
        {
            SerialHeader header = new SerialHeader();
            header.Type = (byte)SerialUtils.TransmissionType.REGISTER;
            header.Subtype = (byte)SerialUtils.RegisterSubtype.GET_INT;

            if (selectedColorHSV == PylonColor.Orange)
            {
                RobotRacer.serialPort.transmitRegData(header, (ushort)Register.RegisterIds.H_HIGH_ORANGE, 0);
                RobotRacer.serialPort.transmitRegData(header, (ushort)Register.RegisterIds.H_LOW_ORANGE, 0);
                RobotRacer.serialPort.transmitRegData(header, (ushort)Register.RegisterIds.S_HIGH_ORANGE, 0);
                RobotRacer.serialPort.transmitRegData(header, (ushort)Register.RegisterIds.S_LOW_ORANGE, 0);
                RobotRacer.serialPort.transmitRegData(header, (ushort)Register.RegisterIds.V_HIGH_ORANGE, 0);
                RobotRacer.serialPort.transmitRegData(header, (ushort)Register.RegisterIds.V_LOW_ORANGE, 0);
                RobotRacer.serialPort.transmitRegData(header, (ushort)Register.RegisterIds.CONV_THRESHOLD_ORANGE, 0);
            }
            else if (selectedColorHSV == PylonColor.Green)
            {
                RobotRacer.serialPort.transmitRegData(header, (ushort)Register.RegisterIds.H_HIGH_GREEN, 0);
                RobotRacer.serialPort.transmitRegData(header, (ushort)Register.RegisterIds.H_LOW_GREEN, 0);
                RobotRacer.serialPort.transmitRegData(header, (ushort)Register.RegisterIds.S_HIGH_GREEN, 0);
                RobotRacer.serialPort.transmitRegData(header, (ushort)Register.RegisterIds.S_LOW_GREEN, 0);
                RobotRacer.serialPort.transmitRegData(header, (ushort)Register.RegisterIds.V_HIGH_GREEN, 0);
                RobotRacer.serialPort.transmitRegData(header, (ushort)Register.RegisterIds.V_LOW_GREEN, 0);
                RobotRacer.serialPort.transmitRegData(header, (ushort)Register.RegisterIds.CONV_THRESHOLD_GREEN, 0);
            }
        }

        public void updateHSVSlider(ushort regId, int value)
        {
            if (hHighSlider.InvokeRequired)
            {
                SetTextCallback4 d = new SetTextCallback4(updateHSVSliderCallback);
                try
                {
                    this.Invoke
                        (d, new object[] { regId, value });
                }
                catch (System.Exception ex) { Console.WriteLine(ex.Message); }
            }
            else
            {
                updateHSVSliderCallback(regId, value);
            }
        }

        public void updateHSVSliderCallback(ushort regId, int value)
        {
            switch (regId)
            {
                case (ushort)Register.RegisterIds.H_HIGH_ORANGE:
                case (ushort)Register.RegisterIds.H_HIGH_GREEN:
                    hHighSlider.Value = value;
                    hRangeLabel.Text = hLowSlider.Value + " - " + hHighSlider.Value;
                    break;
                case (ushort)Register.RegisterIds.H_LOW_ORANGE:
                case (ushort)Register.RegisterIds.H_LOW_GREEN:
                    hLowSlider.Value = value;
                    hRangeLabel.Text = hLowSlider.Value + " - " + hHighSlider.Value;
                    break;
                case (ushort)Register.RegisterIds.S_HIGH_ORANGE:
                case (ushort)Register.RegisterIds.S_HIGH_GREEN:
                    sHighSlider.Value = value;
                    sRangeLabel.Text = sLowSlider.Value + " - " + sHighSlider.Value;
                    break;
                case (ushort)Register.RegisterIds.S_LOW_ORANGE:
                case (ushort)Register.RegisterIds.S_LOW_GREEN:
                    sLowSlider.Value = value;
                    sRangeLabel.Text = sLowSlider.Value + " - " + sHighSlider.Value;
                    break;
                case (ushort)Register.RegisterIds.V_HIGH_ORANGE:
                case (ushort)Register.RegisterIds.V_HIGH_GREEN:
                    vHighSlider.Value = value;
                    vRangeLabel.Text = vLowSlider.Value + " - " + vHighSlider.Value;
                    break;
                case (ushort)Register.RegisterIds.V_LOW_ORANGE:
                case (ushort)Register.RegisterIds.V_LOW_GREEN:
                    vLowSlider.Value = value;
                    vRangeLabel.Text = vLowSlider.Value + " - " + vHighSlider.Value;
                    break;
                case (ushort)Register.RegisterIds.CONV_THRESHOLD_GREEN:
                case (ushort)Register.RegisterIds.CONV_THRESHOLD_ORANGE:
                    convThreshold_trackbar.Value = value;
                    convThreshold_text.Text = value.ToString();
                    break;
        ...
        //public void RobotRacer_KeyUp(object sender, KeyEventArgs e)
        //{
        ...
        internal void xnaRoboRacerGame1_Click(object sender, EventArgs e)
        {
            TheKnack.racer.xnaRoboRacerDisplay1.Focus(); //so any keys pressed do not modify other controls later on when the game exits
            ...
            //stopButton.Enabled = true;
            //changeStateStatus();
            if (Robot_Racers.XNAGame.xnaRoboRacerDisplay.device != null &&
                Robot_Racers.XNAGame.xnaRoboRacerDisplay.device.PixelShader != null &&
                Robot_Racers.XNAGame.xnaRoboRacerDisplay.device.VertexShader != null)
            {
                TheKnack.xnaGame = new XNAGame.xnaGame();
            }
            else
            {
                MessageBox.Show("Issue Displaying in 3D! Check to make sure DirectX 9 and Shaders 1.1 compatible hardware is present.");
            }
        }
        ...
        private void DynamicHSV_xmit_all_Click(object sender, EventArgs e)
        {
            HSVsettings dynamicsettings = new HSVsettings(dynamicHSVTextBox.Text);
            //transmit only the current values to the selected range (orange or green, for the current selected angle)
            byte[] data = new byte[14 * 12]; //plus two bytes for convolution thresholds.
            //populate the data array with the data from dynamicsettings
            int[] intarray = new int[14 * 12];
            dynamicsettings.getArray(intarray);
        ...
        private void saveHSVDialog_FileOk(object sender, CancelEventArgs e)
        {
            TextWriter tw = new StreamWriter(saveHSVDialog.FileName);
            tw.WriteLine(dynamicHSVTextBox.Text);
            tw.Close();
        }

        private void dynamicHSVTextBox_TextChanged(object sender, EventArgs e)
        {
            mirrorHSVtextbox.Text = dynamicHSVTextBox.Text;
        }

        private void saveHSVbutton_Click(object sender, EventArgs e)
        {

            HSVsettings hsvsetting = new HSVsettings(dynamicHSVTextBox.Text);
            if (hsvsetting.hsvRangesValid())
            {
                saveHSVDialog.ShowDialog();
            }
            else
            {
                MessageBox.Show("Cannot Save HSV ranges");
            }

        }

        private void loadHSVButton_Click(object sender, EventArgs e)
        {
            openHSVDialog.ShowDialog();
        }

        private void openHSVDialog_FileOk(object sender, CancelEventArgs e)
        {
            StreamReader sr = new StreamReader(openHSVDialog.FileName);

            string currentHSVString = sr.ReadToEnd();
            sr.Close();

            dynamicHSVTextBox.Text = currentHSVString;
        }

        private void setHSVline_Click(object sender, EventArgs e)
        {
            //need to build the string to set in the text box.  
            string setline = "";

            for (int i = 0; i < 12; i++)
            {
                setline += HSVDataGrid.Rows[0].Cells[i].Value + " ";
            }
            setline += "\r";

            int lineNumber = selectHSVline1.SelectedIndex;
            ...
            string[] hsvLines = dynamicHSVTextBox.Text.Split(splitLine);

            hsvLines[lineNumber] = setline;
            ...
            dynamicHSVTextBox.Text = setline;

        }

        private void getHSVline_Click(object sender, EventArgs e)
        {
            ...
            string[] hsvLines = dynamicHSVTextBox.Text.Split(splitLine);
            string currentline = hsvLines[lineNumber];
            currentline = currentline.Trim();

            string[] lineEntries = currentline.Split(splitSpace);

            for (int i = 0; i < lineEntries.Length; i++)
            {
                HSVDataGrid.Rows[0].Cells[i].Value = lineEntries[i];
            }
        }

        private void setDynamicRangeFromSlider_Click(object sender, EventArgs e)
        {
            //need to build the string to set in the text box.  
            string setline = "";

            if (orangeRadio.Checked == false && greenRadio.Checked == false)
            {
                MessageBox.Show("Please choose either Green or Orange");
            }

            setline += hLowSlider.Value + " ";
            setline += hHighSlider.Value + " ";
            setline += sLowSlider.Value + " ";
            setline += sHighSlider.Value + " ";
            setline += vLowSlider.Value + " ";
            setline += vHighSlider.Value;

            if (orangeRadio.Checked)
            {
                //we need to append the \r
                setline += '\r';
            }
            else
            {
            ...
            int lineNumber = selectHSVline2.SelectedIndex;

            if (lineNumber < 0)
            {
                MessageBox.Show("Please choose an angle range from the list box.");
                return;
            }
            ...
            //now need to replace half of the current line.  Which half depends on whether this is green (first half) or orange (second).
            if (orangeRadio.Checked)
            {
                //replace the second half.
                string[] secondhalf = setline.Split(splitSpace);
                string[] firsthalf = currentline.Split(splitSpace);
                currentline = "";
                currentline += firsthalf[0] + " " + firsthalf[1] + " " + firsthalf[2] + " " + firsthalf[3] + " " + firsthalf[4] + " " + firsthalf[5] + " " + secondhalf[0] + " " + secondhalf[1] + " " + secondhalf[2] + " " + secondhalf[3] + " " + secondhalf[4] + " " + secondhalf[5];
            }
            else
            {
                string[] secondhalf = currentline.Split(splitSpace);
                string[] firsthalf = setline.Split(splitSpace);
                currentline = "";
                currentline += firsthalf[0] + " " + firsthalf[1] + " " + firsthalf[2] + " " + firsthalf[3] + " " + firsthalf[4] + " " + firsthalf[5] + " " + secondhalf[6] + " " + secondhalf[7] + " " + secondhalf[8] + " " + secondhalf[9] + " " + secondhalf[10] + " " + secondhalf[11];


            }

            hsvLines[lineNumber] = currentline;
        ...
        private void setAllDynamicRangeFromSlider_Click(object sender, EventArgs e)
        {
            string setline = "";
        ...
        private void ConvThreshold_trackbar_Scroll(object sender, EventArgs e)
        {
            ushort regId;
            if (selectedColorHSV == PylonColor.Green)
            {
                regId = (ushort)Register.RegisterIds.CONV_THRESHOLD_GREEN;
                greenConvolutionThreshold = convThreshold_trackbar.Value;
            }
            else
            {
                regId = (ushort)Register.RegisterIds.CONV_THRESHOLD_ORANGE;
                orangeConvolutionThreshold = convThreshold_trackbar.Value;
            }
            convThreshold_text.Text = convThreshold_trackbar.Value.ToString();

            SerialHeader header = new SerialHeader();
            header.Type = (byte)SerialUtils.TransmissionType.REGISTER;
            header.Subtype = (byte)SerialUtils.RegisterSubtype.SET_INT;
            RobotRacer.serialPort.transmitRegData(header, regId, convThreshold_trackbar.Value);
        }
