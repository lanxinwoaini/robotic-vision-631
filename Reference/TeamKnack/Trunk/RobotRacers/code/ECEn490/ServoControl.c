...
Configure the base address and OPB clock frequency for the PWM
...
#define SERVO_PERIOD    ((uint32)(SERVO_OPB_FREQ * 0.02   + 0.5))
#define SERVO_DUTY_CTR  ((uint32)(SERVO_OPB_FREQ * 0.0015 + 0.5))
#define SERVO_DISP      ((uint32)(SERVO_OPB_FREQ * 0.0006 + 0.5))
...
void SetServo(int servoNum, int8 position)
{
	...
	else if(position > 64) position = 64;

	duty = SERVO_DUTY_CTR + ((int32)SERVO_DISP * (int32)position) / 64;
