...
XStatus InitGameSystem(void)
{
   ...
   if (Status != XST_SUCCESS)
   {
	...
	XGpio_SetDataDirection(&Gpio, GAME_SYSTEM_GPIO_CHANNEL, 0xFFFFFFF8);
...
uint8 Game_Enabled(void)
{
	...
	return XGpio_DiscreteRead(&Gpio, GAME_SYSTEM_GPIO_CHANNEL) & GAME_ENABLED_M;
}
...
{
	//return GAMEFLAG;
	return XGpio_DiscreteRead(&Gpio, GAME_SYSTEM_GPIO_CHANNEL) & GAME_FLAG_M;
}
...
{
	//return GAMEWTS;
	return XGpio_DiscreteRead(&Gpio, GAME_SYSTEM_GPIO_CHANNEL) & GAME_WAIT_TO_SHOOT_M;
}
