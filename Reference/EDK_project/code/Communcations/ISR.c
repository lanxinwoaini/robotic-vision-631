...
void ISR_GPIO()
{
	...
	if(interruptStatus | GAME_SYSTEM_GPIO_INT)
	{
		...
		if(changes != 0)
		{
			...
			if(changes & GAME_SHOOT_KILL) //Shoot Kill changed
			{ /*dont care*/ }
			if(changes & GAME_SHOOT_PASS) //Shoot Pass changed
			{ /*dont care*/ }
			if(changes & GAME_SHOOT_REVIVE) //Shoot Revive changed
			{ /*dont care*/ }
			if(changes & GAME_ENABLED_M) //Enabled changed
			{
				if(Game_Enabled())
					xil_printf("Truck enabled!\r\n");
				else
					xil_printf("Truck disabled!\r\n");
				Game_ClearShots();
			}
			if(changes & GAME_FLAG_M)
			{
				if(Game_HaveFlag())
					xil_printf("Got flag!\r\n");
				else
					xil_printf("Lost flag!\r\n");
			}
			if(changes & GAME_HIT_M)
			{
				if( (bits & GAME_HIT_M) && (bits & GAME_WAIT_TO_SHOOT_M) )			
					Game_Hit();
				else if( (bits & GAME_HIT_M) && !(bits & GAME_WAIT_TO_SHOOT_M) )
					Game_Paused();
			}
			if(changes & GAME_MISS_M)
			{
				if( (bits & GAME_MISS_M) && (bits & GAME_WAIT_TO_SHOOT_M) )			
					Game_Miss();
				else if( (bits & GAME_MISS_M) && !(bits & GAME_WAIT_TO_SHOOT_M) )
					Game_FlagCaptured();
			}
			if(changes & GAME_WAIT_TO_SHOOT_M)
			{
				if( !(bits & GAME_WAIT_TO_SHOOT_M) ) 
				{
