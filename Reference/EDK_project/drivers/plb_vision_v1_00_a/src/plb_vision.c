...
void Init_Camera()
{
	...
	while(CameraVersion != 0x0 && CameraVersion != 0x823A && CameraVersion != 0x1324)
	{
...
if (DAUGHTER_BOARD == DAUGHTER_BOARD_GAME)
	{
		...
		*/
		...
		/********** COLOR MODE *******************/
		//WriteCameraRegister(0x0F, 0x11);	//set sensor to default color mode
		//WriteCameraRegister(XPAR_PLB_VISION_0_BASEADDR, 0x0F, 0x15);	//set sensor to color & no high dynamic range 
